...
try:
    from triton.language.extra.intel.libdevice import erf, exp, pow, tanh
except ImportError:
    try:
...
def gelu_tanh(x):
    output = (
        0.5 * x * (1 + tanh(x * 0.79788456 * (1 + 0.044715 * tl.float32 * tl.float32)))
...
def gelu_backward_none(x, dy):
    ...
    dydx = (
        scale2 * x_fp32 * exp(-1 * (scale1 * x_fp32 * scale1 * x_fp32))
