...
# This function is roughly a python wrapper of xpuGeneratorImpl::philox_xpu_state in Pytorch.
# https://github.com/pytorch/pytorch/blob/8a4597980c2692b73f35fb3c7145eaeaf2273e77/aten/src/ATen/xpu/xpuGeneratorImpl.cpp#L452
# It returns the current state of the default Philox RNG in seed and offset and
# updates the next offset by adding `increment`.
def philox_xpu_seed_offset(increment, device=None):
    device = device or torch.xpu.current_device()
    gen = torch.xpu.default_generators[device]
