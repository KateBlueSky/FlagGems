...
def test_accuracy_addmm(M, N, K, scalar, dtype):
    mat1 = torch.randn((M, K), dtype=dtype, device="xpu")
    mat2 = torch.randn((K, N), dtype=dtype, device="xpu")
...
def test_accuracy_bmm(M, N, K, dtype):
    batch = 4
    mat1 = torch.randn((batch, M, K), dtype=dtype, device="xpu")
    mat2 = torch.randn((batch, K, N), dtype=dtype, device="xpu")
    ref_mat1 = to_reference(mat1, True)
    ref_mat2 = to_reference(mat2, True)
...
def test_accuracy_mv(M, N, dtype):
    matrix = torch.randn((N, M), dtype=dtype, device="xpu")
    vector = torch.randn((M,), dtype=dtype, device="xpu")
...
def test_accuracy_outer(M, N, dtype):
    inp1 = torch.randn(M, dtype=dtype, device="xpu", requires_grad=True)
    inp2 = torch.randn(N, dtype=dtype, device="xpu", requires_grad=True)
