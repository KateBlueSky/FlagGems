...
def test_accuracy_bitwisenot(shape, dtype):
    if dtype in BOOL_TYPES:
        inp = torch.randint(0, 2, size=shape, dtype=dtype, device="xpu")
    else:
        inp = torch.randint(
...
def test_accuracy_isfinite(shape, dtype):
    if dtype in ALL_FLOAT_DTYPES:
        ...
        inp = torch.randint(-1000, 1000, shape, device="xpu").to(dtype)
    ref_inp = to_reference(inp)
...
def test_accuracy_flip_with_non_dense_input(shape, dtype, dims):
    ...
    if dtype in ALL_FLOAT_DTYPES:
        inp = torch.randn(shape_dialted, dtype=dtype, device="xpu")[::2, ::2]
    else:
        inp = torch.randint(-1000, 1000, shape_dialted, device="xpu").to(dtype)[
            ::2, ::2
        ]
    ref_inp = to_reference(inp, False)
...
def test_accuracy_masked_fill(shape, dtype, threshold):
    inp = torch.zeros(shape, dtype=dtype, device="xpu")
