...
def test_dynamic_function_without_non_tensor_args(use_block_pointer):
    ...
    for ndim in range(8):
        shape = [SIZE] * ndim
        x = torch.randn(shape, device="xpu")
...
def test_dynamic_function_with_broadcasting(use_block_pointer):
    ...
    SIZE = 10
    x = torch.randn([SIZE, 1, SIZE], device="xpu")
    y = torch.randn([1, SIZE, 1], device="xpu")
    alpha = 2.0
    out = axpy(x, y, alpha)
    torch.testing.assert_close(out, alpha * x + y)
...
def test_dynamic_function_with_predefined_out(use_block_pointer):
    ...
    SIZE = 10
    x = torch.randn([SIZE, SIZE, SIZE], device="xpu")
    y = torch.randn([], device="xpu")
    alpha = 2.0
    o = torch.empty([SIZE, SIZE, SIZE], device="xpu")
...
def test_dynamic_function_with_bool_input_and_output(use_block_pointer):
    ...
    SIZE = 10
    x = torch.randn([SIZE, SIZE, SIZE], device="xpu") > 0
    notx = invert(x)
...
def test_dynamic_function_with_nd_buffer(use_1d_tile, use_block_pointer):
    ...
    M, N, K = 40, 60, 80
    x = torch.randn([M, N, K], device="xpu")[::2, ::2, ::2]
    y = torch.randn([N // 2, K // 2, M // 2], device="xpu").permute(2, 0, 1)
    alpha = 2.0
    o = torch.empty([M // 2, N // 2, K // 2], device="xpu")
    out0, out1 = axpyaxmy(x, y, alpha, out0=o)
    assert out0 is o
    torch.testing.assert_close(out0, alpha * x + y)
    torch.testing.assert_close(out1, alpha * x - y)
...
def test_dynamic_function_with_different_stride_order(use_block_pointer):
    ...
    M, N, K = 40, 60, 80
    x = torch.randn([M, N, K], device="xpu")
    y = torch.randn([N, K, M], device="xpu").permute(2, 0, 1)
    alpha = 2.0
    o = torch.empty([M, N, K], device="xpu")
    out0, out1 = axpyaxmy(x, y, alpha, out0=o)
    assert out0 is o
    torch.testing.assert_close(out0, alpha * x + y)
    torch.testing.assert_close(out1, alpha * x - y)
...
def test_dynamic_function_manual_instantiation_mixing_strided_buffer_and_tensor(
    ...
    SIZE = 10
    x = torch.randn([SIZE, SIZE, SIZE], device="xpu")
    y = torch.randn([SIZE, SIZE, SIZE], device="xpu")
    alpha = 2.0
    _out0 = torch.empty([SIZE, SIZE, SIZE], device="xpu")
    _out1 = StridedBuffer(torch.empty([SIZE, SIZE, SIZE], device="xpu"))
...
def test_dynamic_function_manual_instantiation_does_not_support_broadcasting1(
    ...
    SIZE = 10
    x = torch.randn([SIZE, SIZE, SIZE], device="xpu")
    y = torch.randn([1, SIZE], device="xpu")
    alpha = 2.0
    _out0 = torch.empty([SIZE, SIZE, SIZE], device="xpu")
    _out1 = StridedBuffer(torch.empty([SIZE, SIZE, SIZE], device="xpu"))
...
def test_dynamic_function_manual_instantiation_does_not_support_broadcasting2(
    ...
    SIZE = 10
    x = torch.randn([SIZE, SIZE, SIZE], device="xpu")
    y = torch.randn([SIZE, 1, SIZE], device="xpu")
    alpha = 2.0
    _out0 = torch.empty([SIZE, SIZE, SIZE], device="xpu")
    _out1 = StridedBuffer(torch.empty([SIZE, SIZE, SIZE], device="xpu"))
...
@pytest.mark.skipif(
    torch.xpu.get_device_properties(0).total_memory < (80 * 1024**3),
...
def test_dynamic_function_int64_index(use_block_pointer):
    ...
    x = torch.randn((2, 1024, 1024, 1024), dtype=torch.float16, device="xpu")
    y1 = f(x)
    y2 = x * 2.0
